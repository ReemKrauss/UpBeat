...
export const SongPreview = ({ song, onAddFromPlaylist, onSetMiniPlaylist, playerId, provided, removeSong }) => {
    ...
    const isLikedSong = () => {
        return user.likedSongs.some((currsong) => currsong.id === song.id)
    }
    ...
    const getCurrPlaying = () => {
        ...
        if (miniPlaylist.songs[miniPlaylist.currSongIdx].id === song.id) return true
        return false
    }
    ...
    const onMobilePlay = () => {
        if (width < 680) onClickPlay()
    }
    const draggable = provided ? provided : { draggableProps: {}, dragHandleProps: {}, innerRef: () => { } }


    return (<section className="song-preview flex" {...draggable.draggableProps} {...draggable.dragHandleProps} ref={draggable.innerRef} onClick={onMobilePlay} >
        ...
        {onAddFromPlaylist && <button className="add-button" onClick={(ev) => onAddFromPlaylist(ev, song)}>Add</button> || <OptionsMenu song={song} removeSong={removeSong} />}
    </section>)
