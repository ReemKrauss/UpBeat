...
const API_KEY = 'AIzaSyDCjCb3C7AmF1P3HRbRvinplz_XGb5YHXM'
...
async function search(params) {
    ...
    if(!localSearch){
        try {
            ...
            const songs = search.map((res, idx) => ({ id: res.id.videoId, title: res.snippet.title, imgUrl: res.snippet.thumbnails.high.url, duration: durations[idx].contentDetails.duration }))
